...
class VS23S0x0
{
 public:
  ...
  uint32_t piclineByteAddress(int line) {
  ...
  void setPixelYuv(uint16_t xpos, uint16_t ypos, uint8_t color);
  ...
  void digitalIOw(uint8_t pin, uint8_t val) {
    uint8_t bit = (1 << pin);
    if (val)
      m_gpio_state |= bit;
    else
